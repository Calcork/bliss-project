...
class FileCronJobFetcher implements CronJobFetcher
{
    ...
    function isCronJobDue(string $cronjob_name): bool
    {
        ...
        if (!$this->matchesCronExpression($schedule)) {
            return false;
        }

        // Check if already ran this minute
        $report = $this->getReport();
        $last_run = $report['jobs'][$cronjob_name]['last_run'] ?? null;

        if ($last_run !== null) {
            $last_run_minute = intdiv($last_run, 60);
            $current_minute = intdiv(time(), 60);
    ...
    function markCronJobDueStatus(string $cronjob_name, bool $is_due): void
    {
        if ($is_due === false) {
            $report = $this->getReport();
            $report['jobs'][$cronjob_name]['last_run'] = time();
    ...
    private function getJobsData(): array
    {
        if ($this->jobs_data === null) {
            if (!file_exists($this->cronjobs_file)) {
                $this->jobs_data = [];
            } else {
                $this->jobs_data = require $this->cronjobs_file;
            }
        }
    ...
    private function getReport(): array
    {
        ...
        return require $this->report_file;
    }

    private function saveReport(array $report): void
    {
        $dir = dirname($this->report_file);
        if (!is_dir($dir)) {
            mkdir($dir, 0755, true);
        }

        $content = "<?php\n\nreturn " . var_export($report, true) . ";\n";
        file_put_contents($this->report_file, $content);
    }
