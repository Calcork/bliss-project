<?php

namespace App\Controllers\AppController\SystemCall\Testing\Playwright\Tests;

use App\AppCode\Testing\Playwright\PlaywrightTest;
use App\Base\App;
use App\Models\Language;
use App\Models\User;

class ForgotPasswordTest implements PlaywrightTest
{
    public function __construct(private App $app) {}

    public function generateTestingData(): array
    {
        $unique = bin2hex(random_bytes(4));
        $password = 'OldPass123!!';
        $email = "forgot-{$unique}@test.local";

        $em = $this->app->getDoctrine();
        $default_locale = $this->app->getConfig()['app']['default_locale'];
        /** @var Language $language */
        $language = $em->getRepository(Language::class)->findOneBy(['locale' => $default_locale]);

        $user = new User("Forgot User {$unique}", $email, password_hash($password, PASSWORD_BCRYPT), $language);
        $user->setEmailVerifiedAt(new \DateTimeImmutable());
        $em->persist($user);
        $em->flush();

        return [
            'TEST_URL' => $this->app->getEnv()['APP_URL'],
            'TEST_EMAIL' => $email,
            'TEST_NAME' => "Forgot User {$unique}",
            'OLD_PASSWORD' => $password,
            'NEW_PASSWORD' => 'NewPass456!!',
            'BYPASS_KEY' => $this->app->getEnv()['BYPASS_KEY'],
            'MAILHOG_API' => 'http://' . $this->app->getEnv()['MAILHOG_HOST'] . ':8025',
        ];
    }

    public function cleanTestingData(array $data): void
    {
        /** @var string $email */
        $email = $data['TEST_EMAIL'];

        $em = $this->app->getDoctrine();
        $user = $em->getRepository(User::class)->findOneBy(['email' => $email]);

        if ($user !== null) {
            $em->remove($user);
            $em->flush();
        }

        /** @var string $mailhog_api */
        $mailhog_api = $data['MAILHOG_API'];
        @file_get_contents($mailhog_api . '/api/v1/messages', false, stream_context_create([
            'http' => ['method' => 'DELETE'],
        ]));
    }

    public function getTestPath(): string
    {
        return 'app/tests/playwright/forgot-password.spec.js';
    }
}
